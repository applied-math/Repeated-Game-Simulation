...
def excel(data_list):
    ...
    with open(csv_file_path_absolute, "w", newline="") as csvfile:
        csv.writer(csvfile).writerows(
            data_list
        )  # Write rows of data from the list to the CSV file
    print(
        "CSV file saved at:", csv_file_path_absolute
    )  # Notify the user of the CSV file location
    ...
    print(
        f"Excel file created at: {excel_file_path}"
    )  # Notify the user of the Excel file location
...
def prisoner_dilemma(player1_choice, player2_choice):
    ...
    # If player 1 betrays and player 2 cooperates
        ...
        return (
            temptation_reward,
            0,
        )  # Player 1 gets the temptation reward, player 2 gets 0
    ...
    # If player 1 cooperates and player 2 betrays
        ...
        return (
            0,
            temptation_reward,
        )  # Player 1 gets 0, player 2 gets the temptation reward
...
def main():
    ...
    computer_strategy_choice1 = (
        int(input("Enter the number for computer strategy 1: ")) - 1
    )
    computer_strategy1 = strategies[computer_strategy_choice1]

    # Select the second computer's strategy
    computer_strategy_choice2 = (
        int(input("Enter the number for computer strategy 2: ")) - 1
    )
    ...
    player2_score = 0
    computer_choice1 = (
        "cooperate"  # Initialize the first computer's choice to cooperate
    )
    computer_choice2 = (
        "cooperate"  # Initialize the second computer's choice to cooperate
    )
    ...
    for round_num in range(1, rounds + 1):
        ...
        reward_player1, reward_player2 = prisoner_dilemma(
            computer_choice2, computer_choice1
        )
